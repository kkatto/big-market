...
public class RaffleStrategyController implements IRaffleStrategyService {
    ...
    public Response<List<RaffleStrategyRuleWeightResponseDTO>> queryRaffleStrategyRuleWeight(RaffleStrategyRuleWeightRequestDTO request) {
        try {
            ...
            // 2.查询用户已经抽奖的(总)次数
            Integer userActivityAccountTotalUseCount = raffleActivityAccountQuotaService.
                    queryRaffleActivityAccountPartakeCount(request.getUserId(), request.getActivityId());
            // 3.查询规则
            ...
            for (RuleWeightVO ruleWeightVO : ruleWeightVOList) {
                // 转换对象
                List<RaffleStrategyRuleWeightResponseDTO.StrategyAward> strategyAwardList = new ArrayList<>();
                ...
                raffleStrategyRuleWeightResponseDTO.setStrategyAwardList(strategyAwardList);
