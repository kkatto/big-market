...
public class RaffleActivityController implements IRaffleActivityService {
     ...
     */
    @RequestMapping(value = "has_calendar_sign_rebate", method = RequestMethod.POST)
    @Override
    ...
    public Response<Boolean> creditPayExchangeSku(@RequestBody SkuProductShopCartRequestDTO request) {
        try {
            ...
            // 2.支付兑换商品
            String orderId = creditAdjustService.createUserCreditTradeOrder(TradeEntity.builder()
                    .userId(unpaidActivityOrder.getUserId())
                    .tradeName(TradeNameVO.CONVERT_SKU)
                    .tradeType(TradeTypeVO.REVERSE)
                    .tradeAmount(unpaidActivityOrder.getPayAmount().negate())
